# MASTER BOARD — all 43, FINAL v3
**Authoritative.** Rebuilt under the corrected decision rule (`DECISION-RULE-V2.md`): **drive 45% · raw strength incl. presenting 30% · diversification 15% · complementarity 10%.**
Evidence base: brag sheet + résumé + **full LinkedIn experience read to the end** + **every post** + per-person open-web search.

> **THE RULE: take the best player available.** Proven follow-through first, raw strength second, *can they carry a pitch* asked of everyone. Complementarity breaks ties between close players — it is never a reason to take the weaker one. Only hard constraint: don't end up with four of the same person.

**Pool math:** 55 students on the roster (Len says 56) − 12 CEOs = **43 draftable for 48 slots.** The draft doc also names **3 HR + 3 IT Officers**; if they come out of the pool it is **37–38 for 48.** The pool runs out either way.

`DP` = dark pool (invisible to rivals) · `BS` = posted a brag sheet · ⬆⬇ = moved under the corrected rule

| # | Name | Src | D | S | Shape | The evidence |
|---|---|---|---|---|---|---|
| 1 | **Jeda Wannasin** | BS | **9.3** | 5 | Producer/closer | Presidential Scholar; shoots+edits+performs; 529% rev growth; filed a patent app |
| 2 | **Omar Flores** | DP | **9.3** | 2 | All-round | Blank Scholar; o4flo live; President's Office analyst; built the AI analysis nobody asked for |
| 3 | **Brock Alpher** | DP | **9.3** | 3 | Pitch + distribution | Wins pitch comps for cash ($3,500 B.E.T.A.); 150M views; Amazon+eBay; 8 yrs of commerce |
| 4 | **Matthew Ong** | DP | **9.0** | 1 | Resourcer | Coordinated City of Boston permits, 3,000-person festival; cold-contacted 300+ experts |
| 5 | **AJ Bulow** | DP | **9.0** | 1 | Maker | Olin Formula lead engineer 2 yrs; SpaceX; GM. ⚠️ identity unconfirmed |
| 6 | **Tiffany Lukman** | DP | **9.0** | 2 | Builder | 7 yrs on one venture; taught entrepreneurship inside a prison. No external proof |
| 7 | **Will Marra** | BS | **8.8** | 5 | Resourcer/maker | Raised $65K with corporate sponsors, +30% YoY; slept among fridges; SweatGuys live |
| 8 | **Kelly Morgan** | BS | **8.6** | 4 | Operator | 2 yrs 11 mos service work she left off her résumé; pitched a Lisbon C-suite |
| 9 | **David Chen** | DP | **8.6** | 2 | Maker/producer | COR SeniorFriend live and charging ¥299–999; 155 universities; does his own UI |
| 10 | **Tarynea Aggarwal** | BS | **8.5** | 8 | All-round | Published case author; governance over 154 clubs; 3 yrs CWEL; "never leave things to the last minute" |
| 11 | **Zahaan Sabuwala** | BS | **8.4** | 6 | All-round | Three multi-year commitments: 4y2m, 2y8m, 2y. Sold bread into schools at 14 |
| 12 | **Harini Kesamneni** | BS | **8.3** | 7 | Closer | ⬆ Negotiated a 12-acre sale herself; closed a pharma deal in a month; show jumper |
| 13 | **Jack Gramer** | BS | **8.3** | 4 | All-round | Glavin grant → Italy fieldwork → showcase; 2.5 yrs co-founding; builds AI apps |
| 14 | **Mia Mumby** | BS | **8.2** | 4 | Operator | 4 yrs coaching + 240 blankets delivered; offer secured, no fall recruiting. 🔒 **COND 8/31: me or Nuchaba — first pick wins, refuses others** |
| 15 | **Adi Oberoi** | BS | **8.0** | 7 | All-round | 3.92, ranked 1st ×2, FME CEO who turned a profit, runs Premiere. Nandika has him |
| 16 | **Aaron Wu** | DP | **8.0** | 3 | Systems/communicator | ⬆ +0.5 (BIAS-AUDIT). 2y2m self-started fund; 1,130 followers, 98 reactions/post. Scholarships unverified — ask |
| 17 | **Omar Saad** | DP | **8.0** | 1 | Maker | Shipped an iOS app — it's on the App Store. ⚠️ identity unconfirmed |
| 18 | **Anvesha Gupta** | DP | **7.9** | 1 | Operator | Led 10 research fellows for 2 yrs helping small businesses pitch VCs |
| 19 | **Yifei You** | BS | **7.8** | 7 | Technical | Built LLMs to forecast FX pre-ChatGPT; Afora 2y10m. Self-declared follow-through risk |
| 20 | **Anshul Shah** | DP | **7.6** | 3 | Technical | CFA L1 + FINRA SIE; writes production Python and documents it |
| 21 | **Aryaman Gupta** | DP | **7.4** | 2 | Operator | 4 yrs treasurer of a menstrual-education nonprofit; 1y9m startup field outreach |
| 22 | **Ansh Barmecha** | DP | **7.4** | 2 | Events/ops | 2 yrs on a YPO board; helped run Salesforce's 5,000-person World Tour |
| 23 | **Tony Shi** | BS | **7.2** | 5 | Presenter | ⬆ 750+ campus tours, twice weekly; FME CFO **Aug 2024–May 2025 — as a freshman** (résumé-verified; Class of 2028) |
| 24 | **Zeynep Celiktas** | DP | **7.2** | 2 | Operator | ⬇ 2.5 yrs running debate programmes; otherwise 3-month engagements |
| 25 | **Selim Saab** | DP | **7.0** | 3 | Analyst | VC scout at a $180M fund; the only accountant on the board |
| 26 | **Cade Collins** | BS | **7.0** | 5 | Maker/design | bigfootmovers.co live + phone matches — but absent from his own LinkedIn |
| 27 | **Eduardo Camilo** | DP | **7.0** | 1 | Technical | Fish telemetry, offshore wind farms. Max diversification. May take an IT seat |
| 28 | **Mads Syversen** | BS | **7.0** | 5 | Athlete/ops | National slalom + combined champion, 3× All-America. 3 roles absent from LinkedIn; injured |
| 29 | **Ziad El Hout** | BS | **7.0** | 8 | Builder | ⬆ Ran his own e-comm venture 4y8m from ~15; Babson Clo CEO. Storefront now dead |
| 30 | **Yukt Agarwal** | BS | **6.8** | 3 | Operator | ⚠️ Water system and ProjectDAM both absent from his own LinkedIn |
| 31 | **Al Nickerson** | BS | **6.5** | 3 | Creative | D-III distance runner (TFRRS verified) · **bassist in a band with Babson students** (his one-pager; Class of 2027, Marketing). ⚠️ "plays five instruments" retracted 8/31 — not in his materials. See `AUDIT-2026-08-31.md` |
| 32 | **Devi Gupta** | BS | **6.5** | 5 | Operator | ⚠️ LinkedIn is 2 entries; BSBK and the school shop both absent |
| 33 | **Louise Vitienes** | BS | **6.5** | 3 | Resourcer | ⚠️ LinkedIn is 3 entries; Agrifeed absent. Family does own it |
| 34 | **Arnav Aggarwal** | BS | **6.3** | 3 | Researcher | Sole-authored, DOI'd paper. Braille gardening work |
| 35 | **Henrique Nagem** | BS | **6.3** | 7 | Analyst | Built an AI deal-idea dashboard. Everything belongs to an employer |
| 36 | **Ilan Mayo** | BS | **6.0** | 8 | Analyst | Deepest transaction résumé of any junior. Zero self-started. Trap |
| 37 | **Sophia Montague** | DP | **6.0** | 1 | Different | Construction — Go-Tilt, World of Concrete. ⚠️ identity unconfirmed |
| 38 | **Laura Lu** | DP | **6.0** | 4 | BD | Two 18-month roles. "Stealth startup" still unevidenced |
| 39 | **Sudhish Thati** | DP | **5.5** | 1 | Creative? | Zero film evidence anywhere. Don't draft unseen |
| 40 | **Aaryan Mehra** | BS | **5.0** | 4 | Caution | ⚠️ LinkedIn is one entry, no posts, four claims don't check out |
| 41 | **Yohaan Tibrewala** | DP | **5.0** | 1 | Unknown | Empty profile and empty activity |
| 42 | **Hesham Babukhan** | DP | **4.5** | 1 | Unknown | Nothing findable |
| 43 | ~~Rohan Nagpaul~~ | BS | ⛔ | 10 | — | **DO NOT DRAFT.** Five checkable claims fail — SEC filings show ~$1.27M/26 investors vs "$5M AUM" and his name is on neither; SigEp names someone else president; Babson credits Arcangel to two other students; no YC entry; no TEDx |

## HIDDEN GEMS — high desire, low scarcity. The whole edge.
**Matthew Ong (9.0/1) · AJ Bulow (9.0/1) · Omar Flores (9.3/2) · Tiffany Lukman (9.0/2) · David Chen (8.6/2) · Omar Saad (8.0/1) · Anvesha Gupta (7.9/1) · Aryaman Gupta (7.4/2) · Ansh Barmecha (7.4/2) · Zeynep Celiktas (7.2/2) · Eduardo Camilo (7.0/1)** — all dark pool.
**Plus two who posted and hid it:** **Jack Gramer (8.3/4)** wrote nine lines and left out a research grant and a co-founded venture. **Kelly Morgan (8.6/4)** left three years of restaurant work off her own résumé.

## TRAPS — high scarcity, lower true value
**Rohan Nagpaul (10) · Ilan Mayo (8) · Henrique Nagem (7) · Ziad El Hout (8 — better than I first had him, still priced above his record) · Adi Oberoi (7 — genuinely good, but Nandika has a year of history with him and the "$5M contracts" line is unsupported)**

## The only composition check — run it once, at Round 4
Not a slot list. Just one question: **have I ended up with four of the same person?** If all four are analysts, or all four are salespeople, fix it with the last pick. Otherwise take the best player. And if three men are on the board by R4, Len scores gender — Anvesha, Kelly, Mia, Tarynea, Harini and Zeynep are all live.


---
## UPDATE — DRAFT DAY 9 AM (Wed 9/2): SIX COMMITS, FOUR SEATS
**All yes-if-drafted: Jeda · Omar · Mia · Tiffany · Kelly · AJ.** Jeda + Omar additionally refuse ALL other CEOs. The board problem is now allocation, not acquisition:
- **R1 Jeda** (first pick at whatever slot) · **R3 Omar** (invisible; if sniped+forced elsewhere: buy back, open $200K / cap $300K — the only player I pay for).
- **R2 = the clock pick: MIA** — the only commit a rival (Nuchaba, Chi O) can take; if I want her it must be R2. Tiffany (S2), AJ (S1), Kelly (S4) all last to R4 at ~zero risk.
- **Three plans, circled at 1:30 PM after the noon AJ 1:1:** ① R2 Tiffany → R4 AJ (max talent) · ② R2 Mia → R4 Tiffany (balance, 3 women) · ③ R2 Mia → R4 AJ (maker + heart). **Kelly = alternate in every plan.**
- **Capital: do NOT sell down** (reversed — it widens the refused-Jeda window). No buying up. Passes still NO×4.
- Brock: no commit → fallback only. Full detail: `PLAN-POST-COMMITMENT.md` Wed section + sheet p1.

---
## FINAL — 1 PM WED 9/2 (post-AJ 1:1, post-Len-Q&A)
- **AJ 1:1 = A. Dream five: Jeda · Mia · Omar · AJ (+ Kelly if a 5th round).** Personal board: Jeda > AJ ≥ Omar > Mia > Tiffany > Kelly.
- **Len Q&A answers that matter:** hat-draw 1–12 snake; team size uncapped — after R4 a pick comes WITH $400K ("I don't want anyone not picked"); pre-commits explicitly endorsed; HR/IT officers draftable (IT = presentation order → Eduardo flag); trades/mergers run all semester; most money at year-end = highest grade, but "the most important is getting a team."
- **R2 is a door decision:** count rounds on the wall diagram → 5 = Plan A (Mia R2, Omar R3, AJ R4, Kelly R5+$400K); 4 = Plan B (Omar R2, AJ R3, ladder).
- **Buy-backs pre-priced:** Jeda $250→600K · AJ $50→200K · Omar $50→150K · others bargains only.
- Messages: `outreach/DRAFT-DAY-MESSAGES.md`. Printable: `research/DRAFT-DAY-SHEET.pdf` v6.
